...
class NClassesList extends React.Component {
    render() {
        return (
            <ScrollView style={{ flex: 1 }}>
                {this.props.nClasses.map((classData, i) => {
                    return <ListItem
                        key={i}
...
mapStateToProps = state => {
    const { nClasses, studentName } = state.ClassesReducer;
    return { nClasses, studentName }
}
export default connect(mapStateToProps)(NClassesList);