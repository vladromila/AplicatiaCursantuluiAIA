...
class SClassesList extends React.Component {
    render() {
        return (
            <ScrollView style={{ flex: 1 }}>
                {this.props.sClasses.map((classData, i) => {
                    return <ListItem
                        key={i}
...
mapStateToProps = state => {
    const { sClasses, studentName } = state.ClassesReducer;
    return { sClasses, studentName }
}
export default connect(mapStateToProps)(SClassesList);